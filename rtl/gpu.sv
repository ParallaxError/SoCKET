...
(
    ...
    VertexShader vertex_shader_inst (
        .clk        (clk),
        .rst        (rst),
        .in_ready   (in_ready),
        .in_data    (in_data),
    ...
    generate
        for (binner_raster_x = 0; binner_raster_x < NUM_BINS_X; binner_raster_x++) begin
            for (binner_raster_y = 0; binner_raster_y < NUM_BINS_Y; binner_raster_y++) begin
                ...
                SyncFifo #(
                    ...
                    .rd_data_valid  (binner_raster_out_data_valid[binner_raster_x * NUM_BINS_Y + binner_raster_y]),
    ...
    genvar bx, by;
    generate
        for (bx = 0; bx < NUM_BINS_X; bx++) begin
            for (by = 0; by < NUM_BINS_Y; by++) begin
    ...
    genvar fx, fy;
    generate
        for (fx = 0; fx < NUM_BINS_X; fx++) begin
            for (fy = 0; fy < NUM_BINS_Y; fy++) begin
                FragmentShader fragment_shader_inst (
                    .clk        (clk),
                    .rst        (rst),

                    // input streaming iface
                    .in_frag_ready    (raster_frag_out_ready[fx * NUM_BINS_Y + fy]),
                    .in_frag_data     (raster_frag_out_data[fx * NUM_BINS_Y + fy]),
                    .in_frag_valid    (raster_frag_out_valid[fx * NUM_BINS_Y + fy]),
