...
(
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            out_data.x.value <= fixed_point_mult(fixed_point_div(out_mat_data.x.value, out_mat_w), from_real(rendering_pkg::SCREEN_WIDTH));
            // TODO Shorten
            out_data.y.value <= fixed_point_sub(from_real(real'(SCREEN_HEIGHT)), fixed_point_mult(fixed_point_div(out_mat_data.y.value, out_mat_w), from_real(rendering_pkg::SCREEN_HEIGHT)));
